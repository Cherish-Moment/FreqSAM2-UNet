...
if __name__ == '__main__':
    ...
    m = FreqFusion(144, 288).to(device)
    hr_feat = torch.rand(1, 144, 88, 88).to(device)
    lr_feat = torch.rand(1, 288, 44, 44).to(device)
    _, hr_feat, lr_feat = m(hr_feat, lr_feat)

    # 输出测试结果
    print(f"hr_feat_output shape: {hr_feat.shape}")
    print(f"lr_feat_output shape: {lr_feat.shape}")
