...
class FreqSAM2_UNet(nn.Module):
    def __init__(self, resnet_type=50, checkpoint_path=None) -> None:
        super(FreqSAM2_UNet, self).__init__()
        ...
        print(f"self.encoder.blocks中有{num_blocks}个块。")
        self.encoder.blocks = nn.Sequential(
            *blocks
        )

        self.cnn_branch = ResNet(type=50, is_pretrained=False)
    ...
    def forward(self, x):
        ...
        # Sum features from both branches
        x1, x2, x3, x4 = x1 + cnn_x1,cnn_x2 + x2, cnn_x3 + x3, cnn_x4 + x4
